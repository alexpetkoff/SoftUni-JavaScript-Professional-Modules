class RefurbishedSmartphones {
    ...
    sellSmartphone(model, desiredStorage) {
        let isFound = this.availableSmartphones.find(
        ...
        if (isFound === undefined) {
            ...
            if (
                desiredStorage - isFound.storage <= 128 &&
                desiredStorage - isFound.storage > 0
            ) {
    ...
    upgradePhones() {
        if (this.availableSmartphones.length === 0) {
            ...
            this.availableSmartphones.forEach((phone) => {
                phone.storage *= 2;
                array.push(
                    `${phone.model} / ${phone.storage} GB / ${phone.condition
    ...
    salesJournal(criteria) {
        ...
        arraySold.forEach((phone) => {
            resArr.push(
                `${phone.model} / ${phone.storage} GB / ${Number(
                    phone.soldPrice
                ).toFixed(2)}$`
            );
        });
